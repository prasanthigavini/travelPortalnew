...
class AdminContentNotification extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, Request $request = NULL) {
    ...
    $form['admin_content_notification_recepient_fieldset']['admin_content_notification_email'] = [
      '#type' => 'textarea',
      '#title' => $this->t("Email Id\'s to whom the notification is to be sent, add comma seperated emails in case of multiple recipients"),
      '#default_value' => isset($admin_content_notification_email) ? $admin_content_notification_email : $site_email,
      '#description' => $this->t('You can add  maximum 50 emails, leave this field blank if want to send email based on roles.'),
    ];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($user_input_values['admin_content_notification_email'])) {
      ...
      if (count($admin_content_notification_email) > 50) {
        $form_state->setErrorByName('admin_content_notification_email', $this->t('Email Ids should be less than 50.'));
      }
      ...
      if (count($roles_notified)) {
        ...
        if (count($ids) > 50) {
          $form_state->setErrorByName('admin_content_notification_roles_notified', $this->t('User count for the Recipients should be less than 50.'));
        }
      }
    }
  }
